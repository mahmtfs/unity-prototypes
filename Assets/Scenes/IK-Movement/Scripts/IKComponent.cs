...
namespace IK_Movement
{
    public class IKComponent : MonoBehaviour
    {
        ...
        [SerializeField] private float _raycastOriginOffset = 0.3f;
        [SerializeField] private float _IKRaycastDistance = 0.4f;
        ...
        void Update()
        {
            ...
            {
                _playerIKCopy.position = transform.position;
        ...
        void FixedUpdate()
        {
            if (Physics.Raycast(_leftFootBoneRef.position + Vector3.up * _raycastOriginOffset, Vector3.down, out RaycastHit leftHit, _IKRaycastDistance, _movementComponent._groundLayers))
            {
            ...
            if (Physics.Raycast(_rightFootBoneRef.position + Vector3.up * _raycastOriginOffset, Vector3.down, out RaycastHit rightHit, _IKRaycastDistance, _movementComponent._groundLayers))
            {
            ...
            _leftFootGrounded = CheckLeftFootGrounded();
            _rightFootGrounded = CheckRightFootGrounded();
            
            if (_isIdling)
            {
                if (_isGrounded && _rightFootGrounded && _leftFootGrounded)
                {
            ...
            _isGrounded = _movementComponent.IsGrounded(out RaycastHit groundHit);

            if (!_isGrounded && !_leftFootGrounded && !_rightFootGrounded)
            {
        ...
        bool CheckLeftFootGrounded()
        {
            return Physics.Raycast(_leftFootBone.position + Vector3.up * _raycastOriginOffset, Vector3.down, _IKRaycastDistance, _movementComponent._groundLayers);
        }

        bool CheckRightFootGrounded()
        {
            return Physics.Raycast(_rightFootBone.position + Vector3.up * _raycastOriginOffset, Vector3.down, _IKRaycastDistance, _movementComponent._groundLayers);
        }

        public bool IsLeftFootGrounded()
        {
            return _leftFootGrounded;
        }

        public bool IsRightFootGrounded()
        {
            return _rightFootGrounded;
        }
    }
}
